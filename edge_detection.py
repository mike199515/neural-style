...
if __name__ == '__main__': # Test the above code
    ...
    img = cv2.imread("examples/1-content.jpg",cv2.IMREAD_GRAYSCALE) #You need to load an IMG here
    ...
    input_img = tf.placeholder(tf.float32, img.shape)
    x_tensor = tf.expand_dims(tf.expand_dims(input_img, axis=0),-1)
    print(x_tensor.shape)
    edges_tensor = TF_Canny(x_tensor, return_raw_edges=False)
